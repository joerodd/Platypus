...
class NSGAIII_preseed(NSGAIII):
    def initialize(self):
        try:
            e = glob.glob("*.sqlite")
            generations = [int(re.findall("\d+", f)[0]) for f in e]
            max_generation = max(generations)

            os.rename("results_" + str(max_generation) + ".sqlite", "results_" + str(max_generation) + ".sqlite_superseded")

            con = sqlite3.connect("results_" + str(max_generation - 1) + ".sqlite")
            last_complete_generation = max_generation - 1
